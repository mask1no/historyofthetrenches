...
export function BentoGrid() {
  return (
    ...
    >
      <Card className="card-lift md:col-span-6 flex h-full flex-col border-l-4 border-l-accentRed bg-card/90 transition duration-700 ease-out hover:shadow-[0_10px_24px_rgba(196,77,77,0.07)] dark:border-l-accentRed dark:hover:shadow-[0_10px_24px_rgba(0,0,0,0.45)]">
        ...
        <CardContent className="grid flex-1 auto-rows-[minmax(72px,1fr)] gap-3">
          {rugs.map((event, index) => (
            <Link
              ...
              className={`${index > 2 ? "hidden md:flex" : "flex"} h-full items-center justify-between gap-3 rounded-lg border border-accentRed/30 bg-accentRed/5 px-4 py-3 transition duration-500 ease-out hover:border-accentRed/60 hover:bg-accentRed/10 dark:border-accentRed/10`}
            >
          ...
          {rugs.length > 3 && (
            <Link
              href="/timeline"
              className="flex items-center justify-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-muted transition hover:border-accentGold hover:text-fg md:hidden"
            >
      ...
      <Card className="card-lift md:col-span-6 flex h-full flex-col border-l-4 border-l-accentGreen bg-card/90 transition duration-700 ease-out hover:shadow-[0_10px_24px_rgba(47,158,111,0.07)] dark:border-l-accentGreen dark:hover:shadow-[0_10px_24px_rgba(0,0,0,0.45)]">
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="space-y-1">
            <CardTitle className="flex items-center gap-2 text-xl">
        ...
        <CardContent className="grid flex-1 auto-rows-[minmax(72px,1fr)] gap-3">
          {runners.map((event, index) => (
            <Link
              key={event.slug}
              href={`/event/${event.slug}`}
              className={`${index > 2 ? "hidden md:flex" : "flex"} h-full items-center justify-between gap-3 rounded-lg border border-accentGreen/30 bg-accentGreen/5 px-4 py-3 transition duration-500 ease-out hover:border-accentGreen/60 hover:bg-accentGreen/10 dark:border-accentGreen/10`}
            >
              <div className="min-w-0">
                <div className="text-sm font-semibold line-clamp-2">{event.title}</div>
                <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted">
            ...
            <div className="marquee-track">
              <div className="marquee-group">
                {eras.map((era) => (
                  ...
                  >
                    <div className="flex h-full min-h-[130px] md:min-h-[140px] flex-col rounded-2xl border border-border/80 bg-card px-4 py-3 shadow-subtle transition duration-500 ease-out hover:border-accentGold/70 hover:shadow-[0_0_0_1px_rgba(214,177,94,0.4),0_14px_28px_rgba(0,0,0,0.08)] dark:border-transparent dark:shadow-[0_10px_24px_rgba(0,0,0,0.42)] dark:hover:shadow-[0_0_0_1px_rgba(214,177,94,0.4),0_14px_28px_rgba(0,0,0,0.45)]">
